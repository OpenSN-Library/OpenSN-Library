...
func DeleteWebshellRequest(nodeIndex int, webShellID string) error {
	webShellRequestKeyBase := fmt.Sprintf(key.NodeWebshellRequestKeyTemplate, nodeIndex)
	webShellRequestKey := fmt.Sprintf("%s/%s", webShellRequestKeyBase, webShellID)
	_, err := utils.EtcdClient.Delete(context.Background(), webShellRequestKey)
	if err != nil {
		return fmt.Errorf("delete webshell request error: %s", err.Error())
	}
	return nil
}