...
func CreateVethLinkObject(initConfig model.LinkConfig) *VethLink {
	...
	return &VethLink{
		LinkBase: model.LinkBase{
			Enabled:      false,
			CrossMachine: initConfig.EndInfos[0].EndNodeIndex != initConfig.EndInfos[1].EndNodeIndex,
...
func (l *VethLink) Connect() ([]netreq.NetLinkRequest, error) {
	if !l.Enabled {
		logrus.Errorf("Connect %s and %s Error: %s is not enabled", l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID, l.Config.LinkID)
		return nil, fmt.Errorf("%s is not enabled", l.Config.LinkID)
	}

	var requests []netreq.NetLinkRequest

	for i, v := range l.Config.EndInfos {
		if v.InstanceID == "" {
	...
	logrus.Infof(
		...
		l.GetLinkID(),
		l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID,
	)
...
func (l *VethLink) enableSameMachine() error {
	...
	if l.Enabled {
		logrus.Errorf("Enable %s and %s Error: %s has already been enabled", l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID, l.Config.LinkID)
		return fmt.Errorf("%s is enabled", l.Config.LinkID)
	}
	veth := &netlink.Veth{
		LinkAttrs: netlink.LinkAttrs{
			Name:      l.DevInfos[0].Name,
			Namespace: netlink.NsPid(data.InstanceMap[l.Config.EndInfos[0].InstanceID].Pid),
		},
		PeerName:      l.DevInfos[1].Name,
		PeerNamespace: netlink.NsPid(data.InstanceMap[l.Config.EndInfos[1].InstanceID].Pid),
	}
		...
		if ok {
			vxlanDev := netlink.Vxlan{
				LinkAttrs: netlink.LinkAttrs{
					...
					Namespace: netlink.NsPid(data.InstanceMap[l.Config.EndInfos[i].InstanceID].Pid),
					TxQLen:    -1,
				},
				VxlanId:  l.Config.LinkIndex,
				SrcAddr:  data.NodeMap[key.NodeIndex].L3AddrV4,
				Group:    data.NodeMap[l.Config.EndInfos[1-i].EndNodeIndex].L3AddrV4,
				Port:     4789,
				Learning: true,
				L2miss:   true,
				L3miss:   true,
...
func (l *VethLink) Enable() ([]netreq.NetLinkRequest, error) {
	...
	for i, v := range l.Config.EndInfos {
		...
		// if len(l.Config.AddressInfos[i].V4Addr) > 0 {
		// 	setV4AddrReq := netreq.CreateSetV4AddrReq(
		// 		l.DevInfos[i].IfIndex,
		// 		instanceInfo.Pid,
		// 		l.DevInfos[i].Name,
		// 		l.Config.AddressInfos[i].V4Addr,
		// 	)
		// 	requests = append(requests, setV4AddrReq)
		// }
		// if len(l.Config.AddressInfos[i].V6Addr) > 0 {
		// 	setV6AddrReq := netreq.CreateSetV6AddrReq(
		// 		l.DevInfos[i].IfIndex,
		// 		instanceInfo.Pid,
		// 		l.DevInfos[i].Name,
		// 		l.Config.AddressInfos[i].V6Addr,
		// 	)
...
func (l *VethLink) Disable() ([]netreq.NetLinkRequest, error) {
	...
	if !l.Enabled {
		logrus.Errorf(
			...
			l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID, l.Config.LinkID,
		)
		return nil, fmt.Errorf("%s is not enabled", l.Config.LinkID)
	}
	...
	if l.CrossMachine {
		...
		instanceInfo, ok := data.InstanceMap[l.Config.EndInfos[0].InstanceID]
		if !ok {
			logrus.Errorf(
				"Disable Link Between %s and %s Error: Instance %s is not in Node %d",
				l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID, l.Config.EndInfos[0].InstanceID, key.NodeIndex,
			)
			return nil, fmt.Errorf("%s is not in node %d", l.Config.EndInfos[0].InstanceID, key.NodeIndex)
		}
...
func (l *VethLink) SetParameters(para map[string]int64) ([]netreq.NetLinkRequest, error) {
	...
	var requests []netreq.NetLinkRequest
	for i := 0; i < 2; i++ {
		instanceInfo, ok := data.InstanceMap[l.Config.EndInfos[i].InstanceID]
		if !ok {
			continue
		}
	...
	if dirtyConnect {
		if l.Parameter[model.ConnectParameter] == 0 {
			...
			if err != nil {
				logrus.Errorf("Disconnect Link Between %s and %s Error: %s", l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID, err.Error())
				return nil, err
			}
			...
			if err != nil {
				logrus.Errorf("Connect Link Between %s and %s Error: %s", l.Config.EndInfos[0].InstanceID, l.Config.EndInfos[1].InstanceID, err.Error())
				return nil, err
			}
