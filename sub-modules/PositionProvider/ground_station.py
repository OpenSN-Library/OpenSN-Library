...
class GroundStation(Instance):
    ...
    def __init__(self, instance_id: str, ns: str, node_index: int,latitude: float,longitude: float,altitude:float):
        Instance.__init__(self,instance_id, TYPE_GROUND_STATION, ns, node_index)
        self.altitude = altitude
        self.longitude = longitude
        self.latitude = latitude
        self.connected_satellite_id = None
        self.connect_link_id = ""

    def get_closest_satellite(self,satellites : dict[bytes,Instance]) -> (Satellite,float):
