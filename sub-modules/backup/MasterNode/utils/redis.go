...
func init() {
	rdb := redis.NewClient(&redis.Options{
		Addr:     fmt.Sprintf("%s:%d", servConfig.RedisAddr, servConfig.RedisPort),
		Password: servConfig.RedisPassword, // no password set
		DB:       servConfig.RedisDBIndex,  // use default DB
	})
	RedisClient = rdb
}
